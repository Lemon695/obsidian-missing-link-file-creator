...
import { TjDocFile, TJDocFileArgument } from "./tj-doc-file";
...
function generate_jsdoc_return(
	...
	try {
		// 检查是否有内容节点
		if (returnSection.content.nodes.length === 0) return "";

		const childNodes = returnSection.content.nodes[0].getChildNodes();
		if (childNodes.length === 0) return "";

		// 确保节点是DocPlainText类型
		const textNode = childNodes[0];
		if (textNode instanceof DocPlainText) {
...
function generate_jsdoc_arguments(
	...
	try {
		...
		const args = blocks.map((block) => {
			...
			try {
				const childNodes = block.content.getChildNodes();
				if (childNodes.length > 0) {
					const subChildNodes = childNodes[0].getChildNodes();
					description = subChildNodes
						.filter((x): x is DocPlainText => x instanceof DocPlainText)
						.map(x => x.text)
						.join(" ");
				}
			} catch (err) {
				console.error(`Error parsing description for parameter ${name}:`, err);
			}
		...
		console.error("Error generating JSDoc arguments:", error);
		return [];
	}
}
